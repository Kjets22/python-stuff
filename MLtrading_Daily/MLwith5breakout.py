...
MODEL_PATH = 'best_lstm_model_5breakouts.keras'      # Path to save/load the LSTM model
PREDICTIONS_LOG = 'predictions_log.csv'   # Log file for predictions
TRAIN_CHUNK_SIZE = 2000                   # Number of rows per training chunk
...
def read_header(file_path):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            headers = header_line.split('\t')  # The file is tab-separated this gets the header info
            return headers
    except Exception as e:
...
def label_breakouts(df, min_price_change=0.005, max_opposite_change=0.001, time_window=120):
    ...
    for step in range(1, time_window + 1):
        ...
        weak_upward_breakout = (price_change >= 0.0025) & \
                               ((min_price - df['close_1min']) / df['close_1min'] >= -0.0005)
        
        weak_downward_breakout = (price_change <= -0.0025) & \
                                 ((max_price - df['close_1min']) / df['close_1min'] <= 0.0005)
        
        df.loc[weak_upward_breakout, 'breakout_type'] = 3
        df.loc[weak_downward_breakout, 'breakout_type'] = 4
...
def make_prediction(model, scaler, features, predictions_log_df):
    ...
    X_reshaped = X_input.reshape(-1, len(features))
    X_df = pd.DataFrame(X_reshaped, columns=features)
    X_input_scaled = scaler.transform(X_df).astype(np.float32)    
    ...
    class_names = ['No Breakout', 'Upward Breakout', 'Downward Breakout', 'Weak Upward Breakout', 'Weak Downward Breakout']
    logging.info(f"Prediction at {current_time}: {class_names[predicted_class]} ({confidence:.2f}%)")
...
def end_of_day_summary(predictions_log_df):
    ...
    def generate_plots():
        try:
            ...
            plt.xticks(ticks=[0,1,2,3,4], labels=['No Breakout', 'Upward Breakout', 'Downward Breakout', 'Weak Upward Breakout', 'Weak Downward Breakout'])
