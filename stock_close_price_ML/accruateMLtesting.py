...
comparison = pd.DataFrame({
    ...
    'Predicted_High_TF': predictions_tf[:, 0],
    'Predicted_Low_TF': predictions_tf[:, 1]
})
...
for index, row in sampled_comparison.iterrows():
    ...
    print(f"  Actual High: {row['Actual_High']}, Predicted High XGBoost: {row['Predicted_High_XGBoost']}, Predicted High TF: {row['Predicted_High_TF']}")
    print(f"  Actual Low: {row['Actual_Low']}, Predicted Low XGBoost: {row['Predicted_Low_XGBoost']}, Predicted Low TF: {row['Predicted_Low_TF']}")
    print()

# Calculate accuracy within ±10%
accuracy_high_xgb = np.mean(np.abs((comparison['Actual_High'] - comparison['Predicted_High_XGBoost']) / comparison['Actual_High']) <= 0.1) * 100
accuracy_low_xgb = np.mean(np.abs((comparison['Actual_Low'] - comparison['Predicted_Low_XGBoost']) / comparison['Actual_Low']) <= 0.1) * 100

accuracy_high_tf = np.mean(np.abs((comparison['Actual_High'] - comparison['Predicted_High_TF']) / comparison['Actual_High']) <= 0.1) * 100
accuracy_low_tf = np.mean(np.abs((comparison['Actual_Low'] - comparison['Predicted_Low_TF']) / comparison['Actual_Low']) <= 0.1) * 100

# Print accuracy
print(f"High Prediction Accuracy XGBoost: {accuracy_high_xgb:.2f}%")
print(f"Low Prediction Accuracy XGBoost: {accuracy_low_xgb:.2f}%")
print(f"High Prediction Accuracy TensorFlow: {accuracy_high_tf:.2f}%")
print(f"Low Prediction Accuracy TensorFlow: {accuracy_low_tf:.2f}%")

# Plot accuracy
plt.figure(figsize=(10, 6))
categories = ['High_XGBoost', 'Low_XGBoost', 'High_TF', 'Low_TF']
accuracies = [accuracy_high_xgb, accuracy_low_xgb, accuracy_high_tf, accuracy_low_tf]

plt.bar(categories, accuracies, color=['blue', 'blue', 'orange', 'orange'])
plt.ylim(90, 100)
plt.ylabel('Accuracy (%)')
