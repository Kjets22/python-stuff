...
# Create an empty list to store the options data frames
options_data_frames = []

# Iterate over each date in the date range and fetch the options data
date_range = pd.date_range(start=start_date, end=end_date, freq='B')
for date in date_range:
    options_df = get_options_data_frame(api_key, symbol, date.strftime('%Y-%m-%d'))
    if not options_df.empty:
        options_data_frames.append(options_df)

# Combine all the options data frames into a single DataFrame
all_options_data = pd.concat(options_data_frames, ignore_index=True)

# Write the combined DataFrame to a CSV file
all_options_data.to_csv('options_data.csv', index=False)

print(f"Options data written to 'options_data.csv'.")
